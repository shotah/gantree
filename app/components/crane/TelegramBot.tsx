  ...
  return (
      ...
      summary={
        snap.bot?.username
          ...
          : snap.tokenSet
            ...
            : (
                <span className="text-mark">no token</span>
              )
      }
    ...
    >
      <p className="mb-3 text-xs text-faint">
        ...
        {" "}
        <code className="text-dim">TELEGRAM_ALLOWED_USERS</code>
        {" "}
        — recreate after save.
      </p>
      ...
      {!snap.tokenSet
        ? (
            <div className="space-y-3">
              <p className="text-sm text-mark">Paste TELEGRAM_BOT_TOKEN in Secrets, then refresh.</p>
            ...
            <>
              <p className="mb-3 text-sm text-body">
                {snap.bot?.username
                      ...
                      <>
                        ...
                        {snap.link
                          ? (
                              <>
                                {" · "}
                                <a className="text-mark underline" href={snap.link} target="_blank" rel="noreferrer">
                                  open on phone
                                </a>
                              </>
                            )
                  ...
                  : (
                      <span className="text-mark">{snap.detail || "could not reach Telegram"}</span>
                    )}
              </p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <HintField label="name" {...HINTS.tgName}>
                  <input
                    className="rounded border border-line bg-canvas px-2 py-1 text-sm text-fg"
                    maxLength={64}
                    value={name}
                    disabled={readOnly}
                ...
                <HintField label="description (empty chat)" className="sm:col-span-2" {...HINTS.tgDescription}>
                  <textarea
                    className="min-h-16 rounded border border-line bg-canvas px-2 py-1 text-sm text-fg"
                    maxLength={512}
                    value={description}
                    disabled={readOnly}
                ...
                <HintField label="commands" className="sm:col-span-2" {...HINTS.tgCommands}>
                  <textarea
                    className="min-h-20 rounded border border-line bg-canvas px-2 py-1 font-mono text-sm text-fg"
                    value={commands}
                    disabled={readOnly}
      ...
      {snap.tokenSet
        ? (
            <div className="mt-5 border-t border-line pt-4">
              <h3 className="text-xs font-medium uppercase tracking-wide text-dim">Fresh thread</h3>
              <p className="mt-1 text-xs text-faint">
                <code className="text-dim">/new</code>
                {" "}
                distills the chat into
                <code className="text-dim">SELF.md</code>
                , then
                drops history. Telegram will not let this console send as her — she gets a one-tap
                {" "}
                <code className="text-dim">/new</code>
                {" "}
                in that DM. Container RAM is gantry + MCP children, not the image; a long
                thread is prompt tokens, extra tools stay until you revoke.
              </p>
              {allow.length === 0
                ? (
                    <p className="mt-2 text-xs text-faint">save an allowlist id first</p>
                  )
                : (
      ...
      <div className="mt-5 border-t border-line pt-4">
        ...
        <p className="mt-1 text-xs text-faint">
          ...
          message @userinfobot. Later ids show up here after they talk (slog
          {" "}
        ...
        {operatorNew.length
          ? (
              <div className="mt-2">
                ...
                <p className="mt-0.5 text-[11px] text-faint">from Profile — save allowlist, then recreate</p>
                  ...
                  {operatorNew.map((o) => (
                    <li key={`${o.label}:${o.id}`}>
                      ...
                      >
                        ...
                        {" "}
                        {o.label}
                        <span className="ml-1 text-dim">{o.id}</span>
                      </button>
                    </li>
                  ))}
                </ul>
