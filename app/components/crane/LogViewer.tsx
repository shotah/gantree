...
export function LogViewer({ slug, src }: { slug?: string; src?: string }) {
  ...
  const base = src ?? (slug ? `/api/gantries/${encodeURIComponent(slug)}/logs` : "");
  ...
  useEffect(() => {
    ...
    yardFetch(`${base}?tail=200`)
      ...
      .catch(() => undefined)
      .finally(() => {
        if (cancelled || skipFollow) {
          return;
        }
        es = new EventSource(`${base}?tail=0&follow=1`);
        es.onmessage = (ev) => {
          try {
              ...
              return (
                <div key={key} className="mb-1">
                  ...
                  >
                    {shut ? "▸" : "▾"}
                    {" "}
                    {g.turnId ? `turn ${g.turnId}` : "untagged"}
                    {" "}
                    ·
                    {" "}
                    {g.lines.length}
