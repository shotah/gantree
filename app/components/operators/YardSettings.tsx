...
import { PendantPane } from "./PendantPane";
...
export function YardSettings() {
  ...
  const [pane, setPane] = useState<"people" | "yard" | "pendant">("people");
  ...
  return (
    <section className="flex flex-col gap-8" data-shot="settings">
      ...
      <div className="flex gap-1 border-b border-line pb-px">
        ...
        {admin
          ? (
              <button
                type="button"
                className={`rounded-t px-3 py-1.5 text-sm ${pane === "pendant" ? "border border-b-transparent border-edge bg-canvas text-fg" : "text-dim hover:text-body"}`}
                onClick={() => setPane("pendant")}
              >
                Pendant
              </button>
            )
      ...
      {pane === "pendant" && admin ? <PendantPane /> : null}
