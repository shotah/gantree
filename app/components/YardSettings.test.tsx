...
describe("YardSettings", () => {
  it("shows the three roles and an add form for an admin", async () => {
    vi.mocked(yardFetch).mockImplementation(async (input: RequestInfo | URL) => {
      ...
      return {
        json: async () => ({
          ...
          you: { id: "1", name: "kit", displayName: "kit", role: "admin", crane: null, createdAt: "now" },
        }),
      } as Response;
    });
    render(<YardSettings />);
    ...
    expect(screen.getByText(/one crane/)).toBeTruthy();
    expect(screen.getByText(/look — board/)).toBeTruthy();
  ...
  it("hides add/remove for readonly", async () => {
    vi.mocked(yardFetch).mockResolvedValue({
      json: async () => ({
        operators: [{ id: "2", name: "look", displayName: "look", role: "readonly", crane: null, createdAt: "now" }],
        you: { id: "2", name: "look", displayName: "look", role: "readonly", crane: null, createdAt: "now" },
      }),
    } as Response);
    render(<YardSettings />);
    await waitFor(() => expect(screen.getByText("Settings")).toBeTruthy());
