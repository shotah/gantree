...
| yard `gantree.db` / `var/gantree.db` | Yes. Operator hashes, session hashes, samples, audit, GitHub PAT for tools-fetch, pendant Cloudflare token and Worker secrets map. Gitignored. Not a crane’s `data/gantry.db`. |
